...
function seed(n) {
  const u = (a, b) => Math.imul(a, b) >>> 0,
    key = [];
  for (let v = n || 0; v > 0; v = Math.floor(v / 0x100000000))
    key.push(v & 0xffffffff);
  if (!key.length) key.push(0);
  mt[0] = 19650218;
  for (idx = 1; idx < 624; ++idx)
    mt[idx] = (u(1812433253, mt[idx - 1] ^ (mt[idx - 1] >>> 30)) + idx) >>> 0;
  let i = 1,
    j = 0;
  for (let k = Math.max(624, key.length); k > 0; --k, ++i, ++j) {
    ...
    if (j >= key.length) j = 0;
    mt[i] =
      ((mt[i] ^ u(mt[i - 1] ^ (mt[i - 1] >>> 30), 1664525)) + key[j] + j) >>> 0;
  }
...
function int32() {
  if (idx >= 624) {
    for (let k = 0; k < 624; ++k) {
      ...
      mt[k] =
        (mt[(k + 397) % 624] ^ (y >>> 1) ^ (y & 1 ? 0x9908b0df : 0)) >>> 0;
    }
    idx = 0;
  }
  let y = mt[idx++];
...
function gauss(mu = 0, sigma = 1) {
  // Box-Muller with cached spare (matches Python)
  let z = _gauss_next;
  _gauss_next = null;
  if (z === null) {
...
function shuffle(arr) {
  for (let i = arr.length - 1; i > 0; --i) {
    // Fisher-Yates via getrandbits (matches Python)
...
function choices(population, weights) {
  // bisect on cumulative weights (matches Python)
